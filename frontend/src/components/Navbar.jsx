    ...
    >
        ...
        <nav className="hidden md:flex items-center gap-10 flex-1">
          {links.map((l) => (
            <Link
              ...
              className="text-[11px] uppercase tracking-[0.25em] text-neutral-300 hover:text-[#D4AF37] transition-colors duration-300"
            >
              {l.label}
            </Link>
          ))}
      ...
      {mobileOpen && (
        <div className="md:hidden glass border-t border-[#D4AF37]/20 px-6 py-6 space-y-5" data-testid="mobile-menu">
          ...
          <a
            href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Hi Amazing Groups, I'd like to enquire about a BULK / CORPORATE order.")}`}
            target="_blank"
            rel="noopener noreferrer"
