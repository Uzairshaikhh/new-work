...
const CategoryGrid = ({ categories = [], loading = false }) => {
  ...
  if (categories.length === 0) {
    return (
      <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-2xl" data-testid="categories-empty">
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5" data-testid="categories-grid">
      {categories.map((c) => (
        <Link
          key={c.id}
          to={`/category/${c.id}`}
          className="group bg-white border border-gray-100 rounded-2xl p-4 card-shadow card-shadow-hover flex flex-col"
          data-testid={`category-card-${c.id}`}
        >
          <div className="relative aspect-square rounded-xl overflow-hidden bg-amber-cream mb-4">
            <img
          ...
          <h3 className="font-display text-lg text-navy mb-2">{c.name}</h3>
